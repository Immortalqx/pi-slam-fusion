...
class MatcherBoWHomography : public Matcher
{
public:
    MatcherBoWHomography(){
        _estimator=GSLAM::Estimator::create();
        if(!_estimator)
        {
            _estimator=createEstimatorInstance();
            if(!_estimator)
                LOG(ERROR)<<"MatcherBoWHomography failed to create Estimator.";
        }
    }
    ...
    virtual bool match4triangulation(const GSLAM::FramePtr& ref,const GSLAM::FramePtr& cur,
                                     std::vector<std::pair<int,int> >& matches)const
    {
        GSLAM::ScopedTimer mTimer("MatcherBoWHomography::match4triangulation");
    ...
    {
        GSLAM::ScopedTimer mTimer("MatcherBoWHomography::findMatchWindow");
...
bool MatcherBoWHomography::match4initialize(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                  std::vector<std::pair<int,int> >& matches)const
{
    GSLAM::ScopedTimer mtimer("MatcherBoWHomography::match4initialize");
    ...
    float maxDistance=-1;
    bool crossCheck=svar.GetInt("MatcherBoWHomography.CrossCheck",1);
    while(it1 != end1 && it2 != end2)
    {
        if(it1->first == it2->first)
        {
...
//                    if(curFrame->getKeyPointObserve(i2)) continue;
    ...
    if(svar.GetInt("MatcherBoWHomography.FilterAngle"))
    {
...
#ifdef HAS_OPENCV
        if(svar.GetInt("DebugMatcherBoWHomography"))
        {
    ...
    if( 1 )
    {
        if(lastKF->observationNum()==0||curFrame->observationNum()!=0)
        {
            ...
            _estimator->findHomography(H, points1, points2, GSLAM::RANSAC, 4, &mask);
        }
    }
...
REGISTER_MATCHER(MatcherBoWHomography,bow_homography);
