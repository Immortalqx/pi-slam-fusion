...
public:
    ...
    virtual SPtr<GSLAM::FrameConnection> getParent(GSLAM::FrameID parentId)const{
        auto it=_parents.find(parentId);
        if(it==_parents.end()) return SPtr<GSLAM::FrameConnection>();
        else return it->second;
    }
    virtual SPtr<GSLAM::FrameConnection> getChild(GSLAM::FrameID childId)const{
        auto it=_children.find(childId);
        if(it==_children.end()) return SPtr<GSLAM::FrameConnection>();
        else return it->second;
    }
    virtual bool    getParents(std::map<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> >& parents)const{parents=_parents;return true;}
    virtual bool    getChildren(std::map<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> >& children)const{children=_children;return true;}
    virtual bool    addParent(GSLAM::FrameID parentId,const SPtr<GSLAM::FrameConnection>& parent){_parents[parentId]=parent;return true;}
    virtual bool    addChildren(GSLAM::FrameID childId,const SPtr<GSLAM::FrameConnection>& child){_children[childId]=child;return true;}
...
private:
    ...
    // Mapframe data
    int                                 _flags;
    std::map<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> > _parents,_children;
