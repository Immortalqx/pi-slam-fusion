...
class MatcherBoWH : public Matcher
{
public:
    MatcherBoWH(){
        ...
        if(!_estimator)
        {
            ...
            if(!_estimator)
                LOG(ERROR)<<"MatcherBoWH failed to create Estimator.";
        }
    }
    ...
    virtual bool match4triangulation(const GSLAM::FramePtr& ref,const GSLAM::FramePtr& cur,
                                     std::vector<std::pair<int,int> >& matches)const
    {
        GSLAM::ScopedTimer mTimer("MatcherBoWH::match4triangulation");
    ...
    {
        GSLAM::ScopedTimer mTimer("MatcherBoWH::findMatchWindow");
...
bool MatcherBoWH::match4initialize(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                  std::vector<std::pair<int,int> >& matches)const
{
    GSLAM::ScopedTimer mtimer("MatcherBoWH::match4initialize");
    ...
    float maxDistance=-1;
    bool crossCheck=svar.GetInt("MatcherBoWH.CrossCheck",1);
    while(it1 != end1 && it2 != end2)
    {
        if(it1->first == it2->first)
        {
...
//                    if(curFrame->getKeyPointObserve(i2)) continue;
    ...
    if(svar.GetInt("MatcherBoWH.FilterAngle"))
    {
...
#ifdef HAS_OPENCV
        if(svar.GetInt("DebugMatcherBowH"))
        {
    ...
    if( 1 )
    {
        if(lastKF->observationNum()==0||curFrame->observationNum()!=0)
        {
            ...
            _estimator->findHomography(F,points1,points2,GSLAM::RANSAC,1,&mask);
...
REGISTER_MATCHER(MatcherBoWH,bowH);
