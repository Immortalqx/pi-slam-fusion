...
class MatcherFlannH : public Matcher
{
public:
    MatcherFlannH()
    {
        ...
        if(!_estimator)
        {
            LOG(ERROR)<<"MatcherFlannH failed to create Estimator.";
        }
    }
    ...
    {
        GSLAM::ScopedTimer mtimer("MatcherFlannH::match4triangulation");
    ...
    virtual bool match4initialize(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                  std::vector<std::pair<int,int> >& matches)const
    {
        GSLAM::ScopedTimer mtimer("MatcherFlannH::match4initialize");
        ...
        vector<u_char>         mask;
        {
            ...
            _estimator->findHomography(F,points1,points2,GSLAM::RANSAC,4,&mask);
        }
...
REGISTER_MATCHER(MatcherFlannH,flannH);
