...
using std::vector;
    ...
    public:
        MatcherSiftGPU() {
            LOG(INFO) << "MatcherSiftGPU";
            _estimator = GSLAM::Estimator::create();
            if (!_estimator) {
                LOG(ERROR) << "MatcherSiftGPU failed to create Estimator.";
            }
        }

        virtual bool match4triangulation(const GSLAM::FramePtr &ref, const GSLAM::FramePtr &cur,
                                         std::vector<std::pair<int, int> > &matches) const {
            GSLAM::ScopedTimer mtimer("MatcherSiftGPU::match4triangulation");
        ...
        virtual bool match4initialize(const GSLAM::FramePtr &lastKF, const GSLAM::FramePtr &curFrame,
                                      std::vector<std::pair<int, int> > &matches) const {
            GSLAM::ScopedTimer mtimer("MatcherSiftGPU::match4initialize");
            SiftGPU sift;
            SiftMatchGPU *matcher_SiftGPU = new SiftMatchGPU(4096);
            cv::Mat descriptor_mat1 = lastKF->getDescriptor();
            cv::Mat descriptor_mat2 = curFrame->getDescriptor();

            std::vector<float> descriptor1 ;
            descriptor1.reserve(128);
            for (int j = 0; j < descriptor_mat1.cols ; ++j)
            {
                for (int i = 0; i < descriptor_mat1.rows ; ++i)
                {
                    descriptor1.push_back(descriptor_mat1.at<float>(i,j));
                }
            }

            std::vector<float> descriptor2 ;
            descriptor2.reserve(128);
            for (int j = 0; j < descriptor_mat2.cols ; ++j)
            {
                for (int i = 0; i < descriptor_mat2.rows ; ++i)
                {
                    descriptor2.push_back(descriptor_mat2.at<float>(i,j));
                }
            }
...
//            sift->GetFeatureVector(&cur_keypoints[0], &descriptor2[0]);
            LOG(INFO)<<"ref_keypoints: "<<ref_keypoints.size();
            LOG(INFO)<<"cur_keypoints: "<<cur_keypoints.size();
            matcher_SiftGPU->SetDescriptors(0, ref_keypoints.size(), &descriptor1[0]);
            matcher_SiftGPU->SetDescriptors(1, cur_keypoints.size(), &descriptor2[0]);
...
REGISTER_MATCHER(MatcherSiftGPU, SiftGPU);
