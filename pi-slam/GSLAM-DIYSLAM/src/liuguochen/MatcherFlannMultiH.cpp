...
class MatcherFlannMultiH : public Matcher
{
public:
    MatcherFlannMultiH()
    {
        ...
        if(!_estimator)
        {
            ...
            if(!_estimator)
                LOG(ERROR)<<"MatcherFlannMultiH failed to create Estimator.";
        }
    }
...
bool MatcherFlannMultiH::match4initialize(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                     std::vector<std::pair<int,int> >& matches)const
{
    SCOPE_TIMER
...
//    }
    ...
    if (svar.GetInt("MatcherFlannMultiH.FilterAngle", 0))
    {
...
#endif
REGISTER_MATCHER(MatcherFlannMultiH,flann_multiH);
