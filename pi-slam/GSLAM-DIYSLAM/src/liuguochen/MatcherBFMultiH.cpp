...
//        }
    ...
    if(svar.GetInt("MatcherFlannMultiH.PreFilterAngle",1))
    {
        ...
        std::vector<int> *bins;                     // change to pointer for building on windows
    ...
    if (svar.GetInt("MatcherBFMultiH.colmapSolution", 0))
    {
...

        {
            ...
            while (true)
            {
                ...
                double maxError = svar.GetDouble("MatcherBFMultiH.colmapMaxError", 1);
                vector<uchar> maskH;
                vector<uchar> maskF;
                vector<uchar> maskE;
...
#endif
REGISTER_MATCHER(MatcherBFMultiH,bf_multiH);
