...
class InitializerOptSVD : public Initializer
{
public:
...
public:
    InitializerOptSVD(float sigma = svar.GetDouble("InitializerOptSVD.sigma",0.002), int iterations = 200);
...
public:
    ...
    bool useSVD(const std::vector<std::pair<GSLAM::Point2f,GSLAM::Point2f> >& matches,
                const GSLAM::Camera& camera,
                GSLAM::SE3& t12,
                std::vector<std::pair<int,GSLAM::Point3d> >& mpts);
    bool useOpt(const std::vector<std::pair<GSLAM::Point2f,GSLAM::Point2f> >& matches,
                const GSLAM::Camera& camera,
                GSLAM::SE3& t12,
                std::vector<std::pair<int,GSLAM::Point3d> >& mpts);
...
InitializerOptSVD::InitializerOptSVD(float sigma , int iterations)
{
...
bool InitializerOptSVD::initialize(const std::vector<std::pair<GSLAM::Point2f,GSLAM::Point2f> >& matches,
                                const GSLAM::Camera& camera,
                                GSLAM::SE3& t12,
                                std::vector<std::pair<int,GSLAM::Point3d> >& mpts)
{
    GSLAM::SE3 t12SVD, t12Opt;
    std::vector<std::pair<int, GSLAM::Point3d> > mptsSVD, mptsOpt;

    if (useSVD(matches, camera, t12SVD, mptsSVD))
    {
        t12 = t12SVD;
        mpts.swap(mptsSVD);
        return true;
    }
    else if (useOpt(matches, camera, t12Opt, mptsOpt))
    {
        t12 = t12Opt;
        mpts.swap(mptsOpt);
        return true;
...
bool InitializerOptSVD::useOpt(const std::vector<std::pair<GSLAM::Point2f, GSLAM::Point2f> > &matches,
                               const GSLAM::Camera &camera, GSLAM::SE3 &t12,
                               std::vector<std::pair<int, GSLAM::Point3d> > &mpts)
{
    if(!_optimizer){
...
bool InitializerOptSVD::useSVD(const std::vector<std::pair<GSLAM::Point2f, GSLAM::Point2f> > &matches,
                               const GSLAM::Camera &camera, GSLAM::SE3 &t12,
                               std::vector<std::pair<int, GSLAM::Point3d> > &mpts)
{
    mvPn1.clear();
    ...
    thread threadH(&InitializerOptSVD::FindHomography,this,ref(vbMatchesInliersH), ref(SH), ref(H));
    thread threadF(&InitializerOptSVD::FindFundamental,this,ref(vbMatchesInliersF), ref(SF), ref(F));
...
void InitializerOptSVD::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
...
void InitializerOptSVD::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    // Number of putative matches
    const int N = mvPn1.size();
...
void InitializerOptSVD::FindEssential(std::vector<bool> &vbMatchesInliers,float &score,cv::Mat &E21)
{
    // Number of putative matches
...
cv::Mat InitializerOptSVD::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
...
cv::Mat InitializerOptSVD::ComputeF21(const vector<cv::Point2f> &vP1,const vector<cv::Point2f> &vP2)
{
    const int N = vP1.size();
...
float InitializerOptSVD::CheckHomography(const cv::Mat &H21, const cv::Mat &H12, vector<bool> &vbMatchesInliers, float sigma)
{
    const int N = mvPn1.size();
...
float InitializerOptSVD::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma)
{
    const int N = mvPn1.size();
...
bool InitializerOptSVD::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21, cv::Mat &K,
                                  GSLAM::SE3 &t12, std::vector<std::pair<int, GSLAM::Point3d> > &mpts,
                                  float minParallax, int minTriangulated)
{
...
bool InitializerOptSVD::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21, cv::Mat &K,
                                  GSLAM::SE3 &t12, std::vector<std::pair<int, GSLAM::Point3d> > &mpts,
                                  float minParallax, int minTriangulated)
{
    int N=0;
...
void InitializerOptSVD::Triangulate(const cv::Point2f &kp1, const cv::Point2f &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D)
{
    cv::Mat A(4,4,CV_32F);
...
void InitializerOptSVD::Normalize(const vector<cv::Point2f> &vPts, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
    float meanX = 0;
    float meanY = 0;
...
int InitializerOptSVD::CheckRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::Point2f> &vKeys1,
...
void InitializerOptSVD::DecomposeE(const cv::Mat &E, cv::Mat &R1, cv::Mat &R2, cv::Mat &t)
{
    cv::Mat u,w,vt;
...
REGISTER_INITIALIZER(InitializerOptSVD,opt_svd);
