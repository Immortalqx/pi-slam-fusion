...
class MatcherHybird : public Matcher
{
public:
    MatcherHybird(){
    ...
    {
        //if( !match4initialize_bow(lastKF, curFrame, matches) )
        {
            matches.clear();
            return match4initialize_FLANN(lastKF, curFrame, matches);
        }

        return true;
    }

    bool match4initialize_bow(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                      std::vector<std::pair<int,int> >& matches)const
    {
        GSLAM::ScopedTimer mtimer("MatcherHybird::match4initialize_bow");
    ...
    virtual bool match4initialize_FLANN(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                  std::vector<std::pair<int,int> >& matches)const
    {
        GSLAM::ScopedTimer mtimer("MatcherHybird::match4initialize_FLANN");
    ...
    virtual bool match4triangulation(const GSLAM::FramePtr& ref,const GSLAM::FramePtr& cur,
                                     std::vector<std::pair<int,int> >& matches)const
    {
        //FIXME: this function may cause lost, due to wrong matches
        //       so use full function's match

        GSLAM::ScopedTimer mTimer("MatcherHybird::match4triangulation");
        //return match4initialize_bow(ref, cur, matches);
...
REGISTER_MATCHER(MatcherHybird,hybird);
