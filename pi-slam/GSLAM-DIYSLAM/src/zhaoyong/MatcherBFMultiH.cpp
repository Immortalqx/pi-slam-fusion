...
class MatcherBFMultiH : public Matcher
{
public:
    MatcherBFMultiH()
    {
        ...
        if(!_estimator)
        {
            ...
            if(!_estimator)
                LOG(ERROR)<<"MatcherBFMultiH failed to create Estimator.";
        }
    }
...
bool MatcherBFMultiH::match4initialize(const GSLAM::FramePtr& lastKF,const GSLAM::FramePtr& curFrame,
                                          std::vector<std::pair<int,int> >& matches)const
{
    SCOPE_TIMER
    ...
    {
        GSLAM::ScopedTimer tm("match4initialize::flann");
...
//        cv::Ptr<cv::DescriptorMatcher> matcher = cv::makePtr<cv::FlannBasedMatcher>(cv::makePtr<cv::flann::KDTreeIndexParams>());
    ...
    {
        GSLAM::ScopedTimer tm("match4initialize::FilterAngle");
        int binNumberSelect = svar.GetInt("MatcherFlannMultiH.BinNumberSelect", 3);
        int binNumber = svar.GetInt("MatcherFlannMultiH.BinNumber", 30);
        ...
        for (int i = 0; i < matches.size(); ++i)
        {
            ...
            if(selectBin>=binNumber){
                DLOG(ERROR)<<"Angle:"<<angle
                        <<"SelectBin "<<selectBin<<">"<<binNumber;
                selectBin=binNumber-1;
            }
...
//    if (matches.size() < 100) return false;
    ...
    {
        GSLAM::ScopedTimer tm("match4initialize::findFundamental");
        for(auto& m:matches)
        {
            GSLAM::Point2f pt;
        ...
        foundF=_estimator->findFundamental(F,points1,points2,GSLAM::RANSAC,0.01,0.99,&mask);
    }
...
//    }
    ...
    {
        // find multi H
        GSLAM::ScopedTimer tm("match4initialize::multiH");
        std::vector<std::vector<double> > Hs;
        std::vector<std::pair<int,int> >  matchesLeft=matches;

        while(matchesLeft.size()>30&&Hs.size()<5){
        ...
        DLOG(WARNING) << "matchesHSize: " << matchesH.size();
        matches.insert(matches.end(),matchesH.begin(),matchesH.end());
    }



    if (svar.GetInt("MatcherFlannMultiH.FilterAngleGaussian", 0))
    {
        GSLAM::ScopedTimer tm("match4initialize::FilterAngleGaussian");
    ...
    if(svar.GetInt("MatcherFlannMultiH.PreFilterAngle",0))
    {
        GSLAM::ScopedTimer tm("match4initialize::PreFilterAngle");
    ...
    if (svar.GetInt("MatcherBFMultiH.colmapSolution", 0))
    {
        GSLAM::ScopedTimer tm("match4initialize::colmapSolution");
...

        {
            ...
            while (true)
            {
                ...
                double maxError = svar.GetDouble("MatcherBFMultiH.colmapMaxError", 2);
                vector<uchar> maskH;
                vector<uchar> maskF;
                vector<uchar> maskE;
...
REGISTER_MATCHER(MatcherBFMultiH,zy_bfMultiH);
}
