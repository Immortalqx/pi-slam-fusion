...
public:
    ...
    Size(int64_t w, int64_t h) { _width = w; _height = h; }
...
public:
     ...
     */
    virtual Point2_<int64_t> fromLatLngToPixel(double lat, double lng, int zoom)=0;
    /**
     * @brief Get pixel position from lat/lng
     *
     * @param p             - lng(x) / lat(y)
     ...
     * @return pixel position of given lat/lng position
     */
    Point2_<int64_t> fromLatLngToPixel(const PointWithLatLng &p,int zoom)
    {
     ...
     * @return tile index
     */
    virtual Point2_<int64_t> fromLatLngToTileXY(double lat, double lng, int zoom) = 0;

    Point2_<int64_t> fromLatLngToTileXY(const PointWithLatLng& p, int zoom)
    {
    ...
    virtual PointWithLatLng fromPixelToLatLng(int64_t x,int64_t y,int zoom)=0;
    /**
     * @brief Get lat/lng from pixel position
     *
     ...
     * @return current lng(x)/lat(y) position
     */
    PointWithLatLng fromPixelToLatLng(const Point2_<int64_t> &p,int zoom)
    {
     ...
     */
    virtual Point2_<int64_t> fromPixelToTileXY(const Point2_<int64_t> &p)
    {
        return Point2_<int64_t>((int64_t) (p.x / tileSize().width()), (int64_t) (p.y / tileSize().height()));
    }
     ...
     */
    virtual Point2_<int64_t> fromTileXYToPixel(const Point2_<int64_t> &p)
    {
        return Point2_<int64_t>((p.x * tileSize().width()), (p.y * tileSize().height()));
    }
...
protected:
    ...
    constexpr static const int    ZOOMMAX= 25;
...
public:
    ...
    virtual Point2_<int64_t > fromLatLngToPixel(double lat, double lng, int zoom);
    virtual Point2_<int64_t > fromLatLngToTileXY(double lat, double lng, int zoom);
...
inline int PureProjection::getBestZoom(const double &gsd, const double &lat)
{
    ...
    {
        double fz = log(n) / log(2);
        double res = fz - (int)(fz);

        if( res < 0.5 )
            z = (int)(fz);                      // if z is between xxx.00 ~ xxx.50
        else
            z = (int)(fz) + 1;                  // if z is between xxx.50 ~
    }
...
inline Point2_<int64_t > MercatorProjection::fromLatLngToPixel(double lat, double lng, int zoom)
{
    Point2_<int64_t > ret;// = Point.Empty;
    ...
    int64_t mapSizeX = s.width();
    int64_t mapSizeY = s.height();

    ret.x = (int64_t ) clip(x * mapSizeX + 0.5, 0, mapSizeX - 1);
    ret.y = (int64_t ) clip(y * mapSizeY + 0.5, 0, mapSizeY - 1);

    return ret;
}

inline Point2_<int64_t > MercatorProjection::fromLatLngToTileXY(double lat, double lng, int zoom)
{
    Point2_<int64_t > ps = fromLatLngToPixel(lat, lng, zoom);
    return Point2_<int64_t >(ps.x/_tileSize.width(), ps.y/_tileSize.height());
}


inline PointWithLatLng MercatorProjection::fromPixelToLatLng(int64_t x, int64_t y, int zoom)
{
