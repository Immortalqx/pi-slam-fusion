...
/**
 * @brief Svar::update update values in 'i', 'd', 's' to 'data'
 *
 * @param allData       - [in]
 *                          0 - only update exist values in 'data' from 'i', 'd', 's' (Default)
 *                          1 - update all values in 'i', 'd', 's' to 'data'
 */
inline void Svar::update(int allData)
{
    using namespace std;
    ...
    for(SvarWithType<int>::DataIter it=data_i.begin();it!=data_i.end();it++)
    {
        ...
        if( !exist(name) && !allData ) continue;
